...
MODE_STATE_PATH="${MODE_STATE_PATH:-/state/runtime/active-mode.json}"
MODE_VERSIONS_PATH="${MODE_VERSIONS_PATH:-/frameworks/versions.json}"
...
/usr/local/bin/mode-manager \
    ...
    --state-root /state \
    --server-config /server-config/server.cfg \
...
log "Session id: $(date -u +%Y%m%dT%H%M%SZ)-$$"

STEAMCMDDIR="${STEAMCMDDIR:-/home/steam/steamcmd}"
STEAMCLIENT_PATH="${STEAMCLIENT_PATH:-${STEAMCMDDIR}/linux64/steamclient.so}"
if [[ -f "${STEAMCLIENT_PATH}" ]]; then
    mkdir -p ~/.steam/sdk64
    ln -sfT "${STEAMCLIENT_PATH}" ~/.steam/sdk64/steamclient.so || true
fi
