...
VERSIONS_JSON = PROJECT_DIR / "server/frameworks/versions.json"
CS2_DATA_PATH_HOST = os.environ.get("CS2_DATA_PATH", "")
PROJECT_PATH_HOST = os.environ.get("PROJECT_PATH", "")
...
MODE_COMMAND_REPLACEMENTS = {
    "matchzy": {"pause", "unpause"},
    "retakes": {"scramble_teams"},
}
...
def require_auth(fn: Callable):
    ...
    def wrapper(*args, **kwargs):
        if not PASSWORD:
            return Response("PANEL_PASSWORD is not configured", 503)
        if not authorized():
...
def runtime_cfg_path(mode: str) -> Path:
    return RUNTIME_DIR / mode / MODE_DEFS[mode]["startup"]["runtime_cfg"]
...
def verify_mounts() -> dict:
    ...
    for mode, definition in MODE_DEFS.items():
        root = mode_dir(mode)
        mode_cfg = definition["startup"]["mode_cfg"]
        runtime_cfg = definition["startup"]["runtime_cfg"]
        checks[f"{mode}:{mode_cfg}"] = (root / "cfg" / mode_cfg).is_file()
        checks[f"{mode}:{runtime_cfg}"] = (RUNTIME_DIR / mode / runtime_cfg).is_file()
        ...
        for row in mode_defs.declared_mounts(definition):
            source = mode_defs.mount_source_path(row, root, root)
...
def make_backup(job: Job, scope: str = "config") -> Path:
    ...
    state.mkdir()
    for relative in ("server.json", "secrets.json", "console_history.json"):
        source = DATA_DIR / relative
        if source.exists():
            shutil.copy2(source, state / relative)
    for relative in ("modes", "configs", "runtime"):
        source = DATA_DIR / relative
        if source.is_dir():
            shutil.copytree(
                source,
                state / relative,
                ignore=shutil.ignore_patterns("*.tmp", "transactions"),
                dirs_exist_ok=True,
            )
...
def _backup_and_write(path: Path, value) -> str | None:
    ...
    if path.exists():
        try:
            relative = path.resolve().relative_to((DATA_DIR / "configs").resolve())
...
def api_logs():
    ...
    try:
        ...
        container.reload()
        if container_name == GAME_CONTAINER:
            if container.status != "running" or not selected_runtime_mode():
                return jsonify({
                    "ok": True,
                    "source": source,
            ...
            state = read_json(ACTIVE_MODE_JSON, {})
            raw_since = state.get("updated_at") if isinstance(state, dict) else None
            try:
                since = int(datetime.fromisoformat(str(raw_since).replace("Z", "+00:00")).timestamp())
            except (TypeError, ValueError):
                since = int(time.time())
            raw_logs = container.logs(since=since, timestamps=True)
        else:
            raw_logs = container.logs(tail=tail, timestamps=True)
        output = redact(raw_logs.decode("utf-8", errors="replace"))
        if source == "plugin":
            keep = (
                "MatchZy", "Metamod", "CounterStrikeSharp", "AutoReady", "Retakes",
                "Instadefuse", "HeroShift", "RayTrace", "Warcraft", "PanelBridge",
                "ClutchAnnounce", "[CS2 Manager]",
            )
...
def api_restore():
    ...
    def worker(job):
        ...
        for relative in ("server.json", "secrets.json", "console_history.json"):
            ...
            if source.is_file():
                shutil.copy2(source, DATA_DIR / relative)
        for relative in ("modes", "configs", "runtime"):
            source = state / relative
            if source.is_dir():
                shutil.copytree(source, DATA_DIR / relative, dirs_exist_ok=True)
...
def _launch_panel_applier(job: Job) -> bool:
    ...
    try:
        client.containers.run(
            ...
            working_dir="/work",
            volumes={"/var/run/docker.sock": {"bind": "/var/run/docker.sock", "mode": "rw"}, PROJECT_PATH_HOST: {"bind": "/work", "mode": "ro"}},
        )
        return True
    except DockerException:
        return False
