...
5. Round 5 until the match ends: Terrorists receive an AK-47, Counter-Terrorists randomly receive an M4A4 or M4A1-S, both plus their team pistol. Exactly one active player across both teams is chosen at random each round to receive an AWP instead of their team rifle.

Administrators can retune every stage's weapon pools, round ranges, and the preferred-weapon (AWP) cap by editing `RoundLoadoutSequence` without rebuilding the plugin.
