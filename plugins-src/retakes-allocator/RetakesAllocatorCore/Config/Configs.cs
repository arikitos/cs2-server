...
public record ConfigData
{
    ...
    public Dictionary<CsTeam, List<CsItem>> FullBuyPrimaryWeaponPool { get; set; } = new()
    {
        {CsTeam.Terrorist, new List<CsItem> {CsItem.AK47}},
        {CsTeam.CounterTerrorist, new List<CsItem> {CsItem.M4A1S, CsItem.M4A4}},
    };
    ...
    public bool EnableAutomaticPreferredWeapon { get; set; } = false;
    public CsItem AutomaticPreferredWeapon { get; set; } = CsItem.AWP;
    public int MaxAutomaticPreferredWeaponsPerRound { get; set; } = 1;
    public int MinPlayersForAutomaticPreferredWeapon { get; set; } = 5;
    ...
    public IList<string> Validate()
    {
        ...
        if (MaxAutomaticPreferredWeaponsPerRound < 0)
        {
            throw new Exception("'MaxAutomaticPreferredWeaponsPerRound' cannot be negative");
        }

        if (MinPlayersForAutomaticPreferredWeapon < 0)
        {
            throw new Exception("'MinPlayersForAutomaticPreferredWeapon' cannot be negative");
        }

        if (!WeaponHelpers.IsPreferred(CsTeam.Terrorist, AutomaticPreferredWeapon)
            || !WeaponHelpers.IsPreferred(CsTeam.CounterTerrorist, AutomaticPreferredWeapon))
        {
            throw new Exception("'AutomaticPreferredWeapon' must be valid for both teams");
        }
    ...
    {
        var warnings = new List<string>();
        if (!FullBuyPrimaryWeaponPool.TryGetValue(team, out var weapons) || weapons.Count == 0)
        {
            return warnings;
        }
        ...
        {
            if (!WeaponHelpers.GetPossibleWeaponsForAllocationTypeWithoutConfig(
                    WeaponAllocationType.FullBuyPrimary, team).Contains(weapon))
            {
                throw new Exception($"{weapon} is not a valid FullBuy primary weapon for {team}.");
            }

            if (!UsableWeapons.Contains(weapon))
            {
                throw new Exception($"{weapon} in FullBuyPrimaryWeaponPool.{team} is not in UsableWeapons.");
            }
        }
