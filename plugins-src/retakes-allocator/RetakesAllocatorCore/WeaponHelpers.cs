...
public static class WeaponHelpers
{
    ...
    public static ICollection<CsItem> GetPossibleWeaponsForAllocationTypeWithoutConfig(
        ...
        CsTeam team)
    {
        ...
        return _validWeaponsByTeamAndAllocationType[team][allocationType].ToList();
    }
    ...
    public static ICollection<CsItem> GetWeaponsForRoundType(
        ...
        bool givePreferred,
        CsItem? preferredWeaponOverride = null
    )
    {
        ...
        CsItem? primary;
        if (givePreferred && preferredWeaponOverride is not null)
        {
            primary = preferredWeaponOverride;
        }
        else if (!givePreferred
                 && roundType == RoundType.FullBuy
                 && Configs.GetConfigData().FullBuyPrimaryWeaponPool.TryGetValue(team, out var fullBuyPool)
                 && fullBuyPool.Count > 0)
        {
            primary = Utils.Choice(fullBuyPool.Where(IsUsableWeapon).ToList());
        }
        else
        {
