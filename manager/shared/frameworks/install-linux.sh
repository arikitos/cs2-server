...
# manager/shared/frameworks/versions.json. It never follows an unpinned "latest" release.
set -euo pipefail

FRAMEWORK_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
MANAGER_DIR="$(cd "${FRAMEWORK_DIR}/../.." && pwd)"
...
if [[ -z "${CS2_DATA_DIR:-}" ]]; then
  ...
  [[ -n "${CS2_DATA_PATH:-}" ]] || { echo "CS2_DATA_PATH is not configured" >&2; exit 1; }
  CS2_DATA_DIR="${CS2_DATA_PATH}"
  if [[ "${CS2_DATA_DIR}" =~ ^([A-Za-z]):[/\\](.*)$ ]]; then
    drive="$(printf '%s' "${BASH_REMATCH[1]}" | tr '[:upper:]' '[:lower:]')"
    rest="${BASH_REMATCH[2]//\\//}"
    [[ -d "/mnt/${drive}" ]] && CS2_DATA_DIR="/mnt/${drive}/${rest}" || CS2_DATA_DIR="/${drive}/${rest}"
  fi
fi
...
value = json.load(open(path, encoding="utf-8"))[component].get(field)
...
curl -fL --retry 3 "${MM_URL}" -o "${TMP}/metamod.tar.gz"
...
CSS_URL="$(curl -fsSL --retry 3 -H "Accept: application/vnd.github+json" -H "User-Agent: cs2-server-manager" "${CSS_API}" | python3 -c '
import json,re,sys
release=json.load(sys.stdin); pattern=re.compile(sys.argv[1], re.I)
assets=[a["browser_download_url"] for a in release.get("assets", []) if pattern.search(a.get("name", ""))]
if len(assets) != 1:
    raise SystemExit(f"expected one matching CounterStrikeSharp asset, found {len(assets)}")
print(assets[0])
' "${CSS_PATTERN}")"
echo "Installing pinned CounterStrikeSharp ${CSS_VERSION} ..."
curl -fL --retry 3 "${CSS_URL}" -o "${TMP}/css.zip"
